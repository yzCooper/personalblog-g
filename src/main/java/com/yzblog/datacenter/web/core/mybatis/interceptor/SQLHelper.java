...
public class SQLHelper {
     ...
     */
    public static int getCount(final String sql, final MappedStatement mappedStatement, final Object parameterObject, final BoundSql boundSql)
            throws SQLException {
        final String countSql = StringUtils.join("select count(1) from (", sql, ") tmp_count");
    ...
    public static String generatePageSql(String sql, Page<?> page, Dialect dialect) {
        if (dialect.supportsLimit()) {
            return dialect.getLimitString(sql, page.getOffset(), page.getLimit());
        } else {
            return sql;
        }
    }
